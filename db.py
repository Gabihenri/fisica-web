...
def obter_ou_criar_escola(nome: str, rede: str = "", municipio: str = "", estado: str = "") -> Dict[str, Any]:
    ...
    if _texto(municipio):
        consulta = consulta.eq("municipio", _texto(municipio))
    if _texto(estado):
        consulta = consulta.eq("estado", _texto(estado).upper())
    ...
    payload = {
        "nome": nome,
        "rede": _texto(rede) or None,
        "municipio": _texto(municipio) or None,
        "estado": _texto(estado).upper() or None,
    }
...
def obter_ou_criar_turma(escola_id: str, ano_letivo: int, serie_ano: str, turma: str, turno: str = "", componente_curricular: str = "Física", professor_responsavel: str = "") -> Dict[str, Any]:
    client = get_supabase_client()
    serie_ano = _texto(serie_ano)
    turma = _texto(turma)
    if not serie_ano or not turma:
        raise ValueError("Informe série/ano e turma.")

    existente = _primeiro(
        client.table("turmas").select("*")
        .eq("escola_id", escola_id).eq("ano_letivo", int(ano_letivo))
        .eq("serie_ano", serie_ano).eq("turma", turma)
    ...
    payload = {
        ...
        "serie_ano": serie_ano,
        "turma": turma,
        "turno": _texto(turno) or None,
        "componente_curricular": _texto(componente_curricular) or "Física",
...
def obter_ou_criar_grupo(turma_id: str, codigo_grupo: str, professor_responsavel: str = "", quantidade_participantes: int = 0, data_experimento: Optional[str] = None) -> Dict[str, Any]:
    client = get_supabase_client()
    codigo_grupo = _texto(codigo_grupo) or "Grupo 1"
    data_experimento = data_experimento or date.today().isoformat()

    existente = _primeiro(
        client.table("grupos_experimentais").select("*")
        .eq("turma_id", turma_id).eq("codigo_grupo", codigo_grupo)
        .eq("data_experimento", data_experimento).limit(1).execute().data
    )
    if existente:
        return existente
    ...
    payload = {
        ...
        "professor_responsavel": _texto(professor_responsavel) or None,
        "quantidade_participantes": max(int(quantidade_participantes or 0), 0),
    }
...
def salvar_participantes(grupo_id: str, nomes: List[str]) -> List[Dict[str, Any]]:
    ...
    nomes_validos = [_texto(nome) for nome in nomes if _texto(nome)]

    for indice, nome in enumerate(nomes_validos, start=1):
        codigo = f"P{indice:02d}"
        existente = _primeiro(
            client.table("participantes").select("*")
            .eq("grupo_id", grupo_id).eq("codigo_participante", codigo)
            .limit(1).execute().data
        )
        if existente:
            atualizado = client.table("participantes").update({"nome_exibicao": nome}).eq("id", existente["id"]).execute().data
            salvos.append(_primeiro(atualizado) or existente)
        else:
            payload = {"grupo_id": grupo_id, "codigo_participante": codigo, "nome_exibicao": nome}
            salvos.append(_primeiro(client.table("participantes").insert(payload).execute().data) or payload)
    return salvos


def cadastrar_contexto_escolar(payload: Dict[str, Any]) -> Dict[str, Any]:
    nomes = payload.get("nomes") or []
    escola = obter_ou_criar_escola(payload.get("escola", ""), payload.get("rede", ""), payload.get("municipio", ""), payload.get("estado", ""))
    turma = obter_ou_criar_turma(
        escola["id"], int(payload.get("ano_letivo") or date.today().year), payload.get("serie", ""), payload.get("turma", ""),
        payload.get("turno", ""), payload.get("componente_curricular", "Física"), payload.get("professor_responsavel", ""),
    )
    grupo = obter_ou_criar_grupo(
        turma["id"], payload.get("codigo_grupo", "Grupo 1"), payload.get("professor_responsavel", ""),
        len([n for n in nomes if _texto(n)]),
    )
...
def obter_contexto_grupo(grupo_id: str) -> Optional[Dict[str, Any]]:
    client = get_supabase_client()
    grupo = _primeiro(client.table("grupos_experimentais").select("*").eq("id", grupo_id).limit(1).execute().data)
    if not grupo:
        return None
...
def obter_ou_criar_experimento(grupo_id: str, chave_experimento: str) -> Dict[str, Any]:
    ...
    tipo, titulo = TIPOS_EXPERIMENTO[chave_experimento]
    existente = _primeiro(
        client.table("experimentos").select("*").eq("grupo_id", grupo_id).eq("tipo", tipo)
        .eq("status", "em_andamento").order("created_at", desc=True).limit(1).execute().data
    )
    if existente:
        return existente
    payload = {"grupo_id": grupo_id, "tipo": tipo, "titulo": titulo, "status": "em_andamento", "modo_aquisicao": "manual"}
    return _primeiro(client.table("experimentos").insert(payload).execute().data) or payload
...
def registrar_medicao(grupo_id: str, chave_experimento: str, valores: Dict[str, Any], origem: str = "manual") -> Dict[str, Any]:
    ...
    existentes = client.table("medicoes").select("numero_medicao").eq("experimento_id", experimento["id"]).order("numero_medicao", desc=True).limit(1).execute().data or []
    proximo = int(existentes[0]["numero_medicao"]) + 1 if existentes else 1
    payload = {"experimento_id": experimento["id"], "numero_medicao": proximo, "origem": origem, **valores}
    medicao = _primeiro(client.table("medicoes").insert(payload).execute().data) or payload
...
def listar_medicoes(grupo_id: str, chave_experimento: str) -> List[Dict[str, Any]]:
    ...
    if chave_experimento not in TIPOS_EXPERIMENTO:
        return []
    tipo, _ = TIPOS_EXPERIMENTO[chave_experimento]
    experimento = _primeiro(
        client.table("experimentos").select("id").eq("grupo_id", grupo_id).eq("tipo", tipo)
...
def salvar_resultado(grupo_id: str, chave_experimento: str, estatisticas: Dict[str, Any], interpretacao: str) -> None:
    ...
    payload = {
        ...
        "numero_medidas": int(estatisticas.get("n") or 0),
        "gravidade_referencia": estatisticas.get("gravidade_referencia") or 9.80665,
        "media_gravidade": estatisticas.get("media"),
        "desvio_padrao": estatisticas.get("desvio_padrao"),
        "erro_percentual": estatisticas.get("erro_percentual"),
        "valor_minimo": estatisticas.get("minimo"),
        "valor_maximo": estatisticas.get("maximo"),
        "classificacao_qualidade": estatisticas.get("qualidade"),
